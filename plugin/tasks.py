...
@operation
def allocate_ip(manager_host,tenant_name,manager_username,manager_password,pool_id,**kwargs):
    if pool_id == '':
...
@operation
def unallocate_ip(manager_host,tenant_name,manager_username,manager_password,pool_id,resource_id,**kwargs):
    if pool_id == '':
        ctx.logger.error('pool_id was not provided')
        return;
    if resource_id == '':
